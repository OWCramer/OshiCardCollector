...
export function LoadDeckModal({ isOpen, onClose, hasDeckCards, onLoad }: LoadDeckModalProps) {
  ...
  return (
    <Modal title="Load deck" isOpen={isOpen} onClose={onClose}>
      {!isAuthenticated ? (
        <p className="text-sm opacity-75">Sign in to load saved decks.</p>
      ) : loading ? (
        <p className="text-sm opacity-50">Loading saved decks…</p>
      ) : error ? (
        <p className="text-sm text-red-500">{error}</p>
      ) : decks.length === 0 ? (
        <p className="text-sm opacity-50">No saved decks yet.</p>
      ) : (
